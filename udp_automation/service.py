...
class UdpAutomationService:
    def __init__(self, extension_dir: Path, host: str = "127.0.0.1", port: int = 18793, bus: ControlBusServer | None = None, start_bus: bool = True):
        ...
        self._recent_staged_upload_files: deque[Path] = deque(maxlen=24)
    ...
    def _track_staged_upload_file(self, path: Path) -> None:
        self._recent_staged_upload_files.append(path)

    def _cleanup_tracked_staged_upload_files(self) -> None:
        # Keep the newest staged file for in-flight browser reads and remove older ones.
        while len(self._recent_staged_upload_files) > 1:
            stale = self._recent_staged_upload_files.popleft()
            try:
    ...
    async def stop(self) -> None:
        ...
        if self._start_bus:
            await self.bus.stop()
        self._cleanup_tracked_staged_upload_files()
        while self._recent_staged_upload_files:
            stale = self._recent_staged_upload_files.popleft()
            try:
                stale.unlink(missing_ok=True)
            except Exception:
