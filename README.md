...
- `OPENAI_OAUTH_SCOPE` (default: `openid profile email offline_access api.videos.write`)
...
If you see `Missing scopes: api.videos.write` while generating video, re-authorize so the token includes video scopes. Avoid adding unsupported scopes like `api.read`/`api.write` for this OAuth client, as they can cause `invalid_scope`.
