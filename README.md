...
- `OPENAI_ID_TOKEN_EXCHANGE_ENDPOINT` (optional override for id_token -> API token exchange endpoint; app also tries OIDC-discovered `token_endpoint` plus built-in fallbacks, including OAuth token-exchange form payloads (no JSON fallback) on `/oauth/token`, and retries once after refresh on `token_expired`)
...
For OpenAI prompt generation, the app now prioritizes OAuth browser authorization and exchanges the returned `id_token` for an API token (Codex/ChatGPT-style access path).

In **Model/API Settings**, set Prompt Source to **OpenAI API**, then run Browser Authorization.
Use **Open Provider Login in Browser** to run a full OAuth code+PKCE flow in your system browser. On success, the app auto-fills the OpenAI API token and surfaces detected OpenAI org/project context for diagnostics.
