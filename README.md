...
- `OPENAI_ID_TOKEN_EXCHANGE_ENDPOINT` (optional override for id_token -> API token exchange endpoint; app also tries OIDC-discovered `token_endpoint` plus built-in fallbacks, including OAuth token-exchange form payloads on `/oauth/token`, and retries once after refresh on `token_expired`)
