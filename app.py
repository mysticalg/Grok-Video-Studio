    ...
    def _poll_for_manual_image(self) -> None:
        ...
        def _after_poll(result):
            ...
            if not self.manual_image_video_mode_selected:
                ...
                if prompt_visible or make_video_visible:
                    self._append_log(
                        f"Variant {current_variant}: video option appears ready "
                        f"(promptVisible={prompt_visible}, makeVideoVisible={make_video_visible}); moving to prompt entry."
                    )
                else:
                    self._append_log(
                        f"Variant {current_variant}: video option not confirmed yet (attempt {self.manual_image_video_mode_retry_count}); "
                        "continuing with prompt entry to keep flow moving."
                    )

                self.manual_image_video_mode_selected = True
                self.manual_image_submit_retry_count = 0
                QTimer.singleShot(600, self._poll_for_manual_image)
                return
